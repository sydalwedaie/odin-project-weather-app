...
It's a vanillaJS project with two APIs. Visual Crossing's [Weather API](ttps://www.visualcrossing.com/weather-api/) to get the weather data, and [LocationIQ](https://locationiq.com/) to get city and country names. The script would first fetch the weather data (given a user's query for a place), and feed the coordinates from the response into the LocationIQ API to fetch the location's details.
